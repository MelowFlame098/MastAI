...
<div class="flex h-full w-full flex-col">
	<div class="navbar border-base-300 min-h-12 border-b">
		<div class="flex-1">
			<a class="px-4 font-bold" href="/db/{encodeURIComponent($page.params.database)}">
				{$t("n-table-in-db", {
		...
		<div class="flex-none gap-2">
			...
			{:else}
				...
				<a
					...
					href="/api/db/{encodeURIComponent(
						$page.params.database,
					)}/dump/db-{encodeURIComponent($page.params.database)}.sqlite3"
					target="_blank"
					rel="noreferrer"
				>
					{$t("download")}
