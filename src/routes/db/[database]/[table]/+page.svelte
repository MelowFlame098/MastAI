<script lang="ts">
	...
	let PluginComponent: ComponentType | undefined;

	async function loadPlugin(pluginName: keyof typeof plugins) {
		const module = await plugins[pluginName]();
...
<div class="flex w-full flex-col items-center justify-start gap-4">
	<div class="card w-full">
		<div class="card-body">
			...
			<div>
				<div class="overflow-x-auto">
					<table class="table-sm bg-base-200 table w-full">
						...
						<tbody>
							{#each meta.columns as column (column.name)}
			...
			>
				{#each Object.keys(plugins) as name (name)}
